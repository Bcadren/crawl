...
static int _acquirement_weapon_subtype(bool divine, int & /*quantity*/, int agent)
{
    ...
    for (int i = 0; i < NUM_WEAPONS; ++i)
    {
        ...
        if (!divine && !is_range_weapon(item_considered))
        {
            if (acqweight < 500)
                acqweight = 500;
            // lightweight weapons get unproportionately hit by damage weighting.
            if (weapon_has_flag(i, WPNF_LIGHTWEIGHT))
                                      ...
                                      int /*agent*/)
{
    vector<pair<stave_type, int>> weights = {
        ...
        { STAFF_LIFE,           _skill_rdiv(SK_SUMMONINGS) },
        { STAFF_TRANSMUTATION,  _skill_rdiv(SK_TRANSMUTATIONS) },
        { NUM_STAVES,           5 },
    };
...
#define MAX_ACQ_TRIES 40
    ...
    if (feat_eliminates_items(env.grid(pos))
        && agent > GOD_NO_GOD
        && agent < NUM_GODS)
    {
...
static void _make_acquirement_items()
{
    ...
    for (auto obj_type : rand_acq_classes)
    {
