...
item_def* place_monster_corpse(const monster& mons, bool silent, bool force)
{
    ...
    if (goldify)
    {
        ...
        if (feat_eliminates_items(env.grid(mons.pos())) || feat_is_solid(env.grid(mons.pos())) || mons.type == MONS_LAVA_GLOB)
        {
    ...
    else if (feat_is_solid(env.grid(mons.pos())) && !force)
    {
...
static bool _monster_explodes(const monster &mons) {
    ...
    // messages, etc
    if (mons.has_ench(ENCH_INNER_FLAME) || mons.has_ench(ENCH_ENTROPIC_BURST))
        return true;
    switch (mons.type)
    {
