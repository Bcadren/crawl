...
static void _ouch(actor& target, actor * source, miscast_source_info mc_info, int dam, beam_type flavour, string cause)
{
    ...
    int post_res  = resist_adjust_damage(&target, flavour, dam);

    if (post_res)
        target.beam_effects(flavour, dam, post_res);
    ...
    else
    {
        ...
        ouch(post_res, method, source ? source->mid : MID_NOBODY,
             cause.c_str(), see_source,
             source ? source->name(DESC_A, true).c_str() : nullptr, flavour == BEAM_FIRE);
    }
}
